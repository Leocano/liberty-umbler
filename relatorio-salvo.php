			...
			<table class="table table-striped table-hover table-bordered" id="data-table">
				<thead>
					...
					foreach ($columns as $column) {
						?>
					<th>
						<?=htmlentities($column->nickname_column)?>
					</th>
					<?php
					}
					?>
...
?>
	<script type="text/javascript">
		...
		$("#btn-xls").click(function (event) {
			...
			var file = new File([html], "<?=$name_report?>.xls", {
			...
			"drawCallback": function (settings) {
				...
				api.column(groupRow, {
					...
					if (lastGroup != currentGroup && lastGroup != null) {
						$(rows).eq(i).before(
							'<tr class="group"><td class="text-left" colspan="' + qtd_columns +
							'" style="font-weight: bold; background-color: #D0E9C6;">Total: ' + pad(totalHours) + ':' + pad(
								totalMinutes) + ':00</td></tr>' +
					...
					if (last !== group) {
						$(rows).eq(i).before(
							'<tr role="row" class="group"><td colspan="' + qtd_columns +
							'" style="font-weight: bold; background-color: #C4E3F3;">' + group + '</td></tr>'
						);
				...
				lastRow.after(
					'<tr role="row" class="group"><td class="text-left" colspan="' + qtd_columns +
					'" style="font-weight: bold; background-color: #D0E9C6;">Total: ' + pad(totalHours) + ':' + pad(totalMinutes) +
					':00</td></tr>' +
					'<tr role="row" class="group"><td class="text-left" colspan="' + qtd_columns +
					'" style="font-weight: bold; background-color: #D0E9C6;">Resumo do relatório: ' + pad(finalHours) + ':' + pad(
						finalMinutes) + ':00</td></tr>'
		...
		var table = $('#data-table').DataTable({
			...
			"drawCallback": function (settings) {
				...
				api.column(groupRow, {
					...
					if (last !== group) {
						$(rows).eq(i).before(
							'<tr role="row" class="group info"><td colspan="' + qtd_columns + '" style="font-weight: bold;">' + group +
							'</td></tr>'
						);
