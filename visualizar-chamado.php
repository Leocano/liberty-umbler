...
$desc_subcategory = $dao->getSubcategoryById($ticket[0]->id_subcategory);

$dao = new TimekeepingDAO;
$timekeeping = $dao->getTimekeepingByTicketId($id_ticket);
	...
	<div class="col-xs-12 title">
		...
		<p class="help-block">
			...
			<?=$created?>
				...
				(
				<a data-toggle="modal" data-target="#modal-company" class="info-toggle info-toggle-company" href="#" data-name="<?=$company?>"
				    ...
				    data-phone="<?=$ticket[0]->comp_phone?>" data-cellphone="<?=$ticket[0]->comp_cellphone?>">
					<?=$company?>
				</a>)
		</p>
	</div>
			...
			<div class="well well-lg">
				<div class="row">
					<div class="col-xs-12">
						<div class="row">
							<div class="col-sm-6">
								<h2 class="h4"><strong>Horas Apontadas</strong></h2>
							</div>
								...
								<tbody>
									...
									<?php
										...
										<tr>
											...
											<td>
												<?=substr($time->hours, 0, -3)?>
											</td>
											<td>
												<?=$time->desc_timekeeping_type?>
											</td>
											<td>
												<?=$time->name?>
											</td>
			...
			<div class="well well-lg">
				<div class="row">
					<div class="col-xs-12">
						...
						<?php 
								$dao = new HistoryDAO;
								$history = $dao->getHistoryByTicket($id_ticket);
				...
				<div class="row">
					...
					<div class="col-xs-12 solution-block">
						...
						<?php
								...
								?>
							-
							<a href="p-deletar-anexo-solucao.php?id=<?=$attachment->id_solution_attachment?>&token=<?=$_SESSION['token']?>&ticket=<?=$id_ticket?>">Excluir&nbsp;<i class="fa fa-times"></i></a>
							<?php
									}
									echo "<br>";
								}
...
<?php
require 'modals/atribuir-chamado-modals.php';
	...
	<script type="text/javascript">
		...
		$btnAtribute.on("click", function (event) {
			...
			$formAtribute.ajaxSubmit({
				url: 'p-atribuir-chamado.php',
				type: 'POST',
				success: function (status) {
					if (status == "locked") {
						alert("Consultores com apontamento não podem ser removidos do chamado!");
					} else {
						location.href = "visualizar-chamado.php?id=" + <?=$id_ticket?>;
					}
				}
			});
		});
		...
		$btnTimekeep.on("click", function (event) {
			...
			$formTimekeep.ajaxSubmit({
				url: 'p-adicionar-apontamento.php',
				type: 'POST',
				success: function (status) {
					if (status == "success") {
						location.href = "visualizar-chamado.php?id=" + <?=$id_ticket?> + "&timekeeping=true";
					} else {
						alert(status);
					}
				}
		...
		$btnSolution.on("click", function (event) {
			...
			tinymce.activeEditor.uploadImages(function (success) {
				...
				$formSolution.ajaxSubmit({
					url: 'p-adicionar-solucao.php',
					type: 'POST',
					success: function (status) {
						if (status == "success") {
							location.href = "visualizar-chamado.php?id=" + <?=$id_ticket?> + "&solution=true";
						} else {
							alert(status);
						}
					}
		...
		$btnSolutionAttachments.on("click", function (event) {
			...
			$formSolutionAttachments.ajaxSubmit({
				url: 'p-adicionar-anexo-solucao.php',
				type: 'POST',
				success: function (status) {
					if (status == "success") {
						location.href = "visualizar-chamado.php?id=" + <?=$id_ticket?> + "&solution=true";
