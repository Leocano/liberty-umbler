...
if ($_FILES['attachments']['tmp_name'][0] != "") {
    ...
    $path = "solutions/products/" . $id_ticket;
    if (!is_dir($path)){ 
        mkdir($path);
    }
    ...
    for ($i = 0; $i < $fileCount; $i++) {
        ...
        $dao->createSolutionAttachmentProduct($id_ticket, $name, $new_path);
    }
}

$id_user = $_SESSION['user']->getIdUser();
$dao = new HistoryDAO;
$dao->insertHistoryProduct($id_ticket, $id_user, "Documentação anexada por");
